...
*/ 
	...
	$fileName = "Leger-".$kls."-".$peminatan."-".$kodesmt."-".$tampilta.".xls";
...
<html>
	...
	<h4 align="center">
		TAHUN AJARAN <?php echo $tampilta  ?>
		<br>
		KELAS <?php echo strtoupper($kls)." ".strtoupper($peminatan) ?> 
		<br>
		SMK BAKTI IDHATA
	</h4>
	...
	<table border="1" cellpadding="5">
		<thead>
			<tr>
				...
				<th rowspan="3" class="rotate">Jumlah NA P</th>
				<th rowspan="3" class="rotate">Jumlah NA K</th>
				<th rowspan="3" class="rotate"><div class="rotate">Rangking NA Pengetahuan</div></th>
				<th rowspan="3" class="rotate"><div class="rotate">Rangking NA Keterampilan</div></th>
			</tr>	
			<tr>	
				<?php											
					//nama MP
					...
					while ($datan=mysqli_fetch_array($resultmn)) {
						if (($datan['nama_mp']=='Pendidikan Agama Islam')||($datan['nama_mp']=='Pendidikan Agama Kristen')||($datan['nama_mp']=='Pendidikan Agama Katholik')||($datan['nama_mp']=='Pendidikan Agama Hindu')||($datan['nama_mp']=='Pendidikan Agama Buddha')||($datan['nama_mp']=='Pendidikan Agama Kong Hu Cu')) {
							echo '<th colspan="2" class="rotate" style="height: 120px">Pendidikan Agama</th>';
						}else if (($datan['nama_mp']=='Kewarganegaraan')||($datan['nama_mp']=='PKN')) {
							echo '<th colspan="2" class="rotate" style="height: 120px">Pendidikan Kewarganegaraan</th>';
						}else{
							echo '<th colspan="2" class="rotate" style="height: 120px">'.$datan['nama_mp'].'</th>';
						} 	
					} 
					//lokal
					while ($datal=mysqli_fetch_array($resultml)) {
						echo '<th colspan="2" class="rotate" style="height: 120px">'.$datal['nama_mp'].'</th>';
					}
					//peminatan
					while ($datap=mysqli_fetch_array($resultmp)) {
						echo '<th colspan="2" class="rotate" style="height: 120px">'.$datap['nama_mp'].'</th>';
					}											
				?>											
				<th rowspan="2" class="rotate">Sakit</th>
				<th rowspan="2" class="rotate">Izin</th>
				<th rowspan="2" class="rotate">Alpha</th>
			</tr>	
			<tr>	
				<?php											
					//P&K
				...
				while ($i <= $row) {	
					...
					while ($datan=mysqli_fetch_array($resultmn4)) {
						if (($datan['nama_mp']=='Pendidikan Agama Islam')||($datan['nama_mp']=='Pendidikan Agama Kristen')||($datan['nama_mp']=='Pendidikan Agama Katholik')||($datan['nama_mp']=='Pendidikan Agama Hindu')||($datan['nama_mp']=='Pendidikan Agama Buddha')||($datan['nama_mp']=='Pendidikan Agama Kong Hu Cu')) {
							...
							$data=mysqli_fetch_array($result);
							echo '<th>'.$data['sum(nilai_peng)'].'</th>
								<th>'.$data['sum(nilai_keter)'].'</th>';
						}else if (($datan['nama_mp']=='Kewarganegaraan')||($datan['nama_mp']=='PKN')) {
							$datamp=$datan['nama_mp'];
							$query="select mpelajaran.*, nilai.*, sum(nilai_peng), sum(nilai_keter) from mpelajaran, nilai where mpelajaran.kode_kelas=nilai.kode_kelas and mpelajaran.kode_mp=nilai.kode_mp and nilai.kode_kelas='$kk' and mpelajaran.muatan='Nasional' and mpelajaran.nama_mp='$datamp' and semester='$kodesmt'";
							$result=mysqli_query($koneksi,$query);
